...
serve(async (req) => {
  ...
  try {
    ...
    // Bei fester Vorlage: Subject IMMER aus Template, freier Admin-Text wird IGNORIERT.
    ...
    if (dryRun === true) {
      ...
      return new Response(
        JSON.stringify({ dryRun: true, templateId, subject: effectiveSubject, html: previewHtml }),
    ...
    for (let i = 0; i < drivers.length; i++) {
      ...
      try {
        ...
        // Persönlichen Abmeldelink (HMAC). Ohne echtes Fahrerprofil:
        // KEIN Dummy-Link, KEINE Null-UUID, KEIN mailto-Ersatz – Link wird ausgeblendet.
