...
export interface DriverNewsletterTemplate {
  ...
  // Absätze, die NUR aus <strong>…</strong> bestehen, werden als
  // Abschnitts-Überschrift einer neuen Karte gerendert.
  paragraphs: string[];
}
...
// Erkennt Absätze, die ausschließlich aus einem <strong>…</strong>-Tag bestehen
// und damit als Abschnitts-Überschrift einer neuen Karte dienen.
const HEADER_ONLY = /^\s*<strong>([\s\S]*?)<\/strong>\s*$/i;

function renderCard(headerHtml: string | null, bodyParagraphs: string[]): string {
  const header = headerHtml
    ? `<div style="background:#0d2340;color:#ffffff;padding:10px 16px;font-size:15px;font-weight:700;border-radius:6px 6px 0 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">${headerHtml}</div>`
    : '';
  const body = bodyParagraphs
    .map((p) => `<p style="margin:0 0 10px 0;font-size:15px;line-height:1.65;color:#374151;">${p}</p>`)
    .join('');
  const radius = headerHtml ? '0 0 6px 6px' : '6px';
  return `<div style="border:1px solid #e5e7eb;border-radius:6px;margin:0 0 16px 0;background:#ffffff;">${header}<div style="padding:14px 16px 4px 16px;border-radius:${radius};">${body}</div></div>`;
}
 ...
 * Abschnitte (eingeleitet durch reine <strong>…</strong>-Absätze) werden als
 * optisch getrennte Karten gerendert.
 */
export function buildDriverNewsletterInnerHtml(params: {
  ...
  // Gruppiere Absätze in Karten: jeder reine <strong>-Absatz beginnt eine neue.
  type Section = { header: string | null; body: string[] };
  const sections: Section[] = [{ header: null, body: [] }];
  for (const p of tpl.paragraphs) {
    const m = p.match(HEADER_ONLY);
    if (m) sections.push({ header: m[1], body: [] });
    else sections[sections.length - 1].body.push(p);
  }
  const cards = sections
    .filter((s) => s.header || s.body.length > 0)
    .map((s) => renderCard(s.header, s.body))
    .join('');

  return { innerHtml: `${greeting}${cards}${closing}`, subject: tpl.subject };
}