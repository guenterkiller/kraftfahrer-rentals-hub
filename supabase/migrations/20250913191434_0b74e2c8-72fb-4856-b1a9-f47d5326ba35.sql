...
-- Führerscheinklassen: einfache, robuste Normalisierung (CE vs C+E)
-- (passt zu Abfragen mit upper(replace(...)))
CREATE INDEX IF NOT EXISTS idx_fahrer_profile_fs_norm
  ON public.fahrer_profile ( upper(replace(fuehrerscheinklassen::text, '+', '')) );
